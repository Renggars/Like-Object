  ...
  if (date > 31 || date < 1) {
    ...
    for (let i = 0; i < data.length; i++) {
      if(data[i].type == 'Mobil') {
        let dataPlat = data[i].plat.split(' ')
        let tempPlat = dataPlat[1]
        let length = tempPlat.length - 1
        let cekPlat = tempPlat[length]
        if(cekPlat % 2 == 0 && date % 2 != 0) {
          result ++
        } else if(cekPlat % 2 != 0 && date % 2 == 0) {
          result ++
        }
      }
    } 
